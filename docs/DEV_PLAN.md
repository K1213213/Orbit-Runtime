...
| **W19 ✅** · W20 | **W19 完成（2026-08-30）**：图驱动隔离域——`allocateDomains` 纯函数（影响域闭包 > 阈值自动升 L2 独立域 `iso:<unit>`，可共享节点按确定性分块 `shared:<n>`，闭包小即共置安全——逻辑层已证明互不影响，阈值即进程级爆炸半径契约）；`IsolationDomainManager.syncDomains` 是 diff 而非重建（未变域不重启子进程、移除域 await 释放）；内建纯函数宿主 shim（`node -e` spawn，`ORBIT_DOMAIN_UNITS` 选单元，内核绝不向子进程下发代码）；`DomainChannel` 镜像 PaeChannel——单元工具以 `${unitId}:${tool}` 安装（plan 是划分，unitId 全局唯一故无碰撞），调用走 `capabilityInvoke(DOMAIN_TOOL) → hub → channel → manager → 子进程` 落入 RecordJournal（IO_BOUND/inject），重放无需域或其子进程。新增 `ChannelKind.DOMAIN_TOOL`。单测 20 例（分配/协议/传输含真实子进程/生命周期/同步 diff/路由/网关）+ replay_compat 门禁 2 例（记录时宿主仅进入一次、域消失后重放仍逐字节一致），全量 268 测试绿（内核 246→268），前端 84 例不变，strict 编译零错误。W20：域间事务化调用 + 图驱动域分配进宿主状态 | VISION 升级三 |
