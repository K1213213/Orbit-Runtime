 ...
 *
 * Protocol-qualified for the same reason as the Cordis and isolation-domain
 * counterparts: the package barrel is flat, so per-protocol helpers must not
 * compete for one global name.
 */
export function jsonRpcRemoteErrorOf(response: JsonRpcResponse): JsonRpcErrorObject | null {
