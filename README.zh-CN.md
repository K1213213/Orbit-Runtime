...
| M2 | **确定性重放** —— 记录非确定源（模型响应/种子/时钟），重放零调用精确复现，digest 链对账（DSH 不具备） | 规划中 |
| M3 | **影响域图论内核** —— 故障隔离建模为依赖图可达闭包，附隔离定理 | 规划中 |
| M4 | **成本感知路由** —— 通道声明成本/延迟/质量，智能体按预算调度 | 规划中 |
| M5 | 文档完善、基准测试、社区建设 | 规划中 |
