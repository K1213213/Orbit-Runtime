...
### 波次 7 · v0.7.0 审计哈希链（W30）

VISION §3.1「落盘 + 签名」的签名半落地：append-only 审计日志变为可证明未被篡改。

| 阶段 | 内容 | 依据 |
|---|---|---|
| **W30 ✅** | `core-hub/audit/audit_chain.ts`（HMAC 哈希链：prevHash/chainHash + genesis 种子 + `verifyAuditChain` 纯函数）+ `TraceJournal` 接密钥自动带链 + 宿主 `auditSigningKey` 选项与 `verifyAuditChain()` + strict 档联动（构造期强制签名、boot 验证被篡改链拒绝启动）+ 恢复延续链尾 + `orbit audit` CLI + 控制台审计页链卡 + replay_compat 门禁（签名不扰动重放） | VISION §3.1（见下方进度日志） |

**波次 7 门禁**：无密钥完全不带链字段（v0.6.x 逐字一致）· 篡改/删除/错 key 均被
...
### 波次 10 · v0.10.0 合规报告（W33，PRODUCT_PLAN P2）

审计故事的产品收口：档位 + 审计链 + 治理干预 → 一份可向第三方出示的报告。

| 阶段 | 内容 | 依据 |
|---|---|---|
| **W33 ✅** | lib.js `buildComplianceReport`/`countInterventions` 纯函数（状态诚实推导：EMPTY/UNSIGNED/PASS/FAIL）+ bridge `GET /api/compliance` + `/api/compliance/export?format=md/json/pdf`（复用零依赖 buildPdf，导出本身进审计流）+ 审计页合规报告卡（导出按钮）+ 前端 4 例新测试 | PRODUCT_PLAN P2（见下方进度日志） |
